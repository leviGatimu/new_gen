...
function generateTeacherKey() {
    return "TCH-" . rand(1000, 9999);
}

if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    $name = trim($_POST['full_name']);
    $access_key = generateTeacherKey();
    
    // Temporary password
    $temp_pass = password_hash("teacher123", PASSWORD_DEFAULT);

    try {
        $stmt = $pdo->prepare("INSERT INTO users (full_name, role, access_key, password) VALUES (:name, 'teacher', :key, :pass)");
        $stmt->execute([
            'name' => $name,
            'key' => $access_key,
            'pass' => $temp_pass
        ...
        $success = "Teacher Account Created! <br><strong>Activation Key:</strong> <span style='font-size:1.8rem; color:#FF6600; display:block; margin-top:10px;'>$access_key</span>";
...
<head>
    <meta charset="UTF-8">
    <title>Add Teacher | NGA Admin</title>
    ...
    <style>
        ...
        .card {
            ...
            max-width: 500px; width: 100%; border: 1px solid var(--border); border-top: none;
        }
        ...
        input:focus { border-color: var(--primary); box-shadow: 0 0 0 4px rgba(255,102,0,0.1); }
...
<nav class="top-navbar">
    ...
    <div class="nav-menu">
        ...
        <a href="teachers.php" class="nav-item active"><i class='bx bxs-id-card'></i> <span>Teachers</span></a>
        ...
        <div class="card">
            <?php if($success): ?>
                <div class="status-alert alert-success">
                    ...
                    <?php echo $success; ?>
                    <p style="margin-top:15px; font-size: 0.85rem; opacity: 0.8;">Share this key with the teacher so they can complete their registration.</p>
                </div>
            <?php endif; ?>
            ...
            <form method="POST">
                <div class="form-group">
                    ...
                    <input type="text" name="full_name" required placeholder="e.g. Mr. John Doe" autocomplete="off">
                </div>
                
                <div class="alert-info">
                    <i class='bx bxs-info-circle'></i> 
                    <strong>System Logic:</strong> You only need to generate the key. The teacher will provide their own email and password during activation.
                </div>

                <button type="submit" class="btn-submit">
                    <i class='bx bx-key'></i> Generate Activation Key
                </button>
            </form>
        </div>
    </div>
